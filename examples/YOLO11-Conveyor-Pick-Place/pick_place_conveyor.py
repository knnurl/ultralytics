...
class DetectionPipeline:
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        for idx, box in enumerate(res.boxes):
            ...
            label = f"id={track.track_id} cls={cls_id} rx={rx:.1f} ry={ry:.1f} a={angle:.1f}"
            detections.append(
...
def main(config_path: Path = Path("vision_config.json")):
    ...
    try:
        for frame in stream:
            ...
            output = pipeline.draw(frame, detections) if detections else cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)
